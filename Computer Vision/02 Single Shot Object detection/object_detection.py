...
def detect(frame, net, transform): # We define a detect function that will take as inputs, a frame, a ssd neural network, and a transformation to be applied on the images, and that will return the frame with the detector rectangle.
    height, width = frame.shape[:2] # We get the height and the width of the frame. using shape attribute. H, W, number of chanels 1= b/w, 3 = color r/b/g
    frame_t = transform(frame)[0] # We apply the transformation to our frame to convertfrom numpy array to torch tensor.
    x = torch.from_numpy(frame_t).permute(2, 0, 1) # We convert the frame into a torch tensor.
    # frame_t.permute(2, 0, 1) to convert image is RBG but Neural network is GRB
    x = Variable(x.unsqueeze(0)) # We add a fake dimension corresponding to the batch. 0 is position of the first dimension
    ...
    # first two on upper corner ov vector and second one of bottom corner lower right
    # Detection tensor contains - batch, number of classes that is objects, number of occrence of class, (scrore, x0,y0,x1,y1) for found and not found.
    
    for i in range(detections.size(1)): # For every class: that is number of classes
        j = 0 # We initialize the loop variable j that will correspond to the occurrences of the class.
        while detections[0, i, j, 0] >= 0.3: # We take into account all the occurrences j of the class i that have a matching score larger than 0.6.
            ...
            # using numpy() to convert to numpy array so that we can 
...
net = build_ssd('test') # our neural network object
net.load_state_dict(torch.load('ssd300_mAP_77.43_v2.pth', map_location = lambda storage, loc: storage)) 
# We get the weights of the neural network from another one that is pretrained (ssd300_mAP_77.43_v2.pth).
# SSD is faster than faster RCNN and YOlo
# map_location and location just a way to open the ssd

# Creating the transformation
transform = BaseTransform(net.size, (104/256.0, 117/256.0, 123/256.0)) 
# scalling the color values`
